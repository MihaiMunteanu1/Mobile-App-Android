...
class PendingWorker(ctx:Context, params:WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for(item in appContext.container.database.itemDao().getAll()){
            if(item.requiresUpdate) {
                Log.d("PendingWorker", "REQUIRES UPDATE ${item}")
                ...
                appContext.container.itemRepository.quite_remove(item)
                val res = appContext.container.itemRepository.save(item)
